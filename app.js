...
app.put('/users/:id', authenticateToken, async (req, res) => {
  ...
  if (!email && !title) {
    return res.status(400).json({ message: 'Email or title is required to update' });
  }
  ...
  try {
    ...
    const result = await pool.query(
      `UPDATE users SET ${fields.join(', ')}, updated_at = CURRENT_TIMESTAMP WHERE id = $${fields.length + 1} RETURNING *`,
      values
    );
